...
export default function Navbar({ isLight, onThemeToggle, showNavigation = true }) {
    ...
    <header className="nav premium-nav">
      <div className="nav-inner">
        <a className="brand" href="#top" aria-label={`${site.name} home`} onClick={close}>
          ...
          <span className="brand-copy"><strong>{site.name}</strong><small>Web Experience · Frontend · AI</small></span>
        </a>

        <div className="nav-actions">
          {showNavigation && (
            <button className={`mobile-menu-button ${open ? "is-open" : ""}`} type="button"
          ...
          {showNavigation && (
            <nav className={`nav-links ${open ? "is-open" : ""}`} aria-label="Primary navigation">
              {nav.map((item) => <a href={item.href} key={item.href} onClick={close}>{item.label}</a>)}
            </nav>
          )}
